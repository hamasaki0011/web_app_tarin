...
class WebServer:
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    # 例外発生の場合も、発生しなかった場合も通信を終了させるためTCP通信のcloseを行う
                    client_socket.close()
    ...
    def create_server_socket(self) -> socket: # type: ignore
        ...
        # socketをlocalhostのポート8082番に割り当てる
        server_socket.bind(("localhost", PORT))
        server_socket.listen(10)
        return server_socket # type: ignore
    
    def handle_client(self, client_socket: socket) -> None: # type: ignore
        """
        クライアントと接続済みのsocketを引数として受け取り、
        リクエストを処理してレスポンスを送信する
        """
        # クライアントから送られてきたデータを取得する
        request = client_socket.recv(4096) # type: ignore
        ...
        # クライアントへレスポンスを送信する
        client_socket.send(response) # type: ignore
